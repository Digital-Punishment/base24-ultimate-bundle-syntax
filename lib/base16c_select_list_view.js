...
class Base16cSelectListView extends SelectListView {
  ...
  viewForItem(theme) {
    ...
    if (this.base16.isConfigTheme(theme.scheme, theme.style)) {
  ...
  selectItemView(view) {
    ...
    if (this.attached) { return this.base16.enableTheme(theme.scheme, theme.style); }
  }
  ...
  confirmed(theme) {
    ...
    this.base16.setThemeConfig(theme.scheme, theme.style);
    this.cancel();
  ...
  getThemes() {
    ...
    if (atom.config.get(`${this.base16.packageName}.matchUserInterfaceTheme`)) {
      styles = [atom.config.defaultSettings[`${this.base16.packageName}`].style];
    } else {
      styles = atom.config.getSchema(`${this.base16.packageName}.style`).enum;
    }
    const themes = [];
    schemes.forEach(scheme => styles.forEach(style => themes.push({scheme, style, name: `${scheme} (${style})`})));
    return themes;
  }
}

module.exports = Base16cSelectListView;
